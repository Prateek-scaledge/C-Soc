...
{ 
	...
	write_addr(0x40002003, 0x00008800); //LCR  8th bit set for DLAB enable & 4th bit set for Parity eneable
  write_addr(0x40002001, 0x00000000); //DLM
	write_addr(0x40002000, 0x00000007); //DLL
	write_addr(0x40002003, 0x00000000); //Disable bit
  write_addr(0x40002003, 0x00000800); //LCR
  ...
  // UART write 
  for(j=0;j<7;j++) {
	 write_addr(0x40002000,((j*10)+1)); //THR
	}
	
	write_addr(0x40002000,0xaaaaaaaa); //THR
	write_addr(0x40002000,0x55555555); //THR
	write_addr(0x40002000,0xaaaaaaaa); //THR
	...
	uart_read_store=read_addr(0x40002005);
